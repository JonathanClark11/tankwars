...
void HeightMap::ResetHeights() {
    for (int i = 0; i < sizeof(g_HeightMap); i++) {
        g_HeightMap[i] = 0;
    }
}

void HeightMap::loadRawFile(string filename) {
...
void HeightMap::SetVertexColor(char *pHeightMap, int x, int y) {
...
void HeightMap::RenderHeightMap() {
    ...
    //else
    glBegin( GL_QUADS );                // Render Lines Instead
